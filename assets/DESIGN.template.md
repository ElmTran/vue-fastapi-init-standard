...
Lovable's website radiates warmth through restraint. The entire page sits on a creamy, parchment-toned background (`#f7f4ed`) that immediately separates it from the cold-white conventions of most developer tool sites. This isn't minimalism for minimalism's sake 鈥?it's a deliberate choice to feel approachable, almost analog, like a well-crafted notebook. The near-black text (`#1c1c1c`) against this warm cream creates a contrast ratio that's easy on the eyes while maintaining sharp readability.

The custom Camera Plain Variable typeface is the system's secret weapon. Unlike geometric sans-serifs that signal "tech company," Camera Plain has a humanist warmth 鈥?slightly rounded terminals, organic curves, and a comfortable reading rhythm. At display sizes (48px鈥?0px), weight 600 with aggressive negative letter-spacing (-0.9px to -1.5px) compresses headlines into confident, editorial statements. The font uses `ui-sans-serif, system-ui` as fallbacks, acknowledging that the custom typeface carries the brand personality.

What makes Lovable's visual system distinctive is its opacity-driven depth model. Rather than using a traditional gray scale, the system modulates `#1c1c1c` at varying opacities (0.03, 0.04, 0.4, 0.82鈥?.83) to create a unified tonal range. Every shade of gray on the page is technically the same hue 鈥?just more or less transparent. This creates a visual coherence that's nearly impossible to achieve with arbitrary hex values. The border system follows suit: `1px solid #eceae4` for light divisions and `1px solid rgba(28, 28, 28, 0.4)` for stronger interactive boundaries.

**Key Characteristics:**
- Warm parchment background (`#f7f4ed`) 鈥?not white, not beige, a deliberate cream that feels hand-selected
...
### Primary
- **Cream** (`#f7f4ed`): Page background, card surfaces, button surfaces. The foundation 鈥?warm, paper-like, human.
- **Charcoal** (`#1c1c1c`): Primary text, headings, dark button backgrounds. Not pure black 鈥?organic warmth.
...
- **Cream Surface** (`#f7f4ed`): Card backgrounds, section fills 鈥?same as page background for seamless integration.

### Interactive
- **Ring Blue** (`#3b82f6` at 50% opacity): `--tw-ring-color`, Tailwind focus ring.
- **Focus Shadow** (`rgba(0,0,0,0.1) 0px 4px 12px`): Focus and active state shadow 鈥?soft, warm, diffused.
...
- **Feature**: Variable font with continuous weight axis 鈥?allows fine-tuned intermediary weights like 480.

### Hierarchy

| Role | Font | Size | Weight | Line Height | Letter Spacing | Notes |
|------|------|------|--------|-------------|----------------|-------|
| Display Hero | Camera Plain Variable | 60px (3.75rem) | 600 | 1.00鈥?.10 (tight) | -1.5px | Maximum impact, editorial |
...
- No box-shadow by default 鈥?borders define boundaries
...
- Links: Camera Plain 14鈥?6px weight 400, `#1c1c1c` text
...
- No color change on hover 鈥?decoration carries the interactive signal
...
- Warm, inviting input area 鈥?not clinical
...
- The scale expands generously at the top end 鈥?sections use 80px鈥?08px vertical spacing for editorial breathing room

### Grid & Container
- Max content width: approximately 1200px (centered)
- Hero: centered single-column with massive vertical padding (96px+)
- Feature sections: 2鈥? column grids
- Full-width footer with multi-column link layout
- Showcase sections with centered card grids

### Whitespace Philosophy
- **Editorial generosity**: Lovable's spacing is lavish at section boundaries (80px鈥?08px). The warm cream background makes these expanses feel cozy rather than empty.
- **Content-driven rhythm**: Tight internal spacing within cards (12鈥?4px) contrasts with wide section gaps, creating a reading rhythm that alternates between focused content and visual rest.
...
### Don't
- Don't use pure white (`#ffffff`) as a page background 鈥?the cream is intentional
- Don't use heavy box-shadows for cards 鈥?borders are the containment mechanism
- Don't introduce saturated accent colors 鈥?the palette is intentionally warm-neutral
- Don't use weight 700 (bold) 鈥?600 is the maximum weight in the system
- Don't apply 9999px radius on rectangular buttons 鈥?pills are for icon/action toggles
- Don't use sharp focus outlines 鈥?the system uses soft shadow-based focus indicators
- Don't mix border styles 鈥?`#eceae4` for passive, `rgba(28,28,28,0.4)` for interactive
- Don't increase letter-spacing on headings 鈥?Camera Plain is designed to run tight at scale
...
| Mobile | 600鈥?40px | Standard mobile layout |
| Tablet Small | 640鈥?00px | 2-column grids begin |
| Tablet | 700鈥?68px | Card grids expand |
| Desktop Small | 768鈥?024px | Multi-column layouts |
| Desktop | 1024鈥?280px | Full feature layout |
| Large Desktop | 1280鈥?536px | Maximum content width, generous margins |
...
### Collapsing Strategy
- Hero: 60px 鈫?48px 鈫?36px headline scaling with proportional letter-spacing
- Navigation: horizontal links 鈫?hamburger menu at 768px
- Feature cards: 3-column 鈫?2-column 鈫?single column stacked
- Template gallery: grid 鈫?stacked vertical cards
- Stats bar: horizontal 鈫?stacked vertical
- Footer: multi-column 鈫?stacked single column
- Section spacing: 128px+ 鈫?64px on mobile
...
### Iteration Guide
1. Always use cream (`#f7f4ed`) as the base 鈥?never pure white
...
6. The inset shadow on dark buttons is the signature detail 鈥?don't skip it
